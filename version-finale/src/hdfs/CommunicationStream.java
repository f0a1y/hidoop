...
public class CommunicationStream {
	...
	public FileDataI receiveFileData() throws IOException {
        ...
        int numberDaemons = this.receiveDataInt();
		for (int i = 0; i < numberDaemons; i++) {
			int daemon = this.receiveDataInt();
			data.addDaemon(daemon);
		}
		int numberFragments = this.receiveDataInt();
		data.setNumberFragments(numberFragments);
		return data;
	}
	...
	public KV receiveKV() throws IOException {
		String k = this.receiveDataString();
		String v = this.receiveDataString();
		return new KV (k, v);
	}
	...
	public void sendData(KV data) throws IOException {
		this.sendData(data.k);
		this.sendData(data.v);
	}
