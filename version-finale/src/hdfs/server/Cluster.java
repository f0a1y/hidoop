...
public class Cluster {
	...
	public KV receiveKV(int daemon) throws IOException {
		return this.daemonStreams[daemon].receiveKV();
	}
	...
	public void sendAllData(KV data) throws IOException {
		for (int i = 0; i < this.daemonStreams.length; i++) {
			this.daemonStreams[i].sendData(data);
		}
	}
	
	public void sendAllData(Collection<Integer> daemons, KV data) throws IOException {
		for (Integer daemon : daemons) {
			this.daemonStreams[daemon].sendData(data);
		}
	}
	
	public void sendAllData(int[] daemons, KV data) throws IOException {
		for (int i = 0; i < daemons.length; i++) {
			this.daemonStreams[daemons[i]].sendData(data);
		}
	}
	
	public void sendData(int daemon, KV data) throws IOException {
		this.daemonStreams[daemon].sendData(data);
	}	
	
	public void sendAllDataExcept(Collection<Integer> daemons, KV data) throws IOException {
		for (int i = 0; i < this.numberDaemons; i++) 
			if (!daemons.contains(i))
				this.daemonStreams[i].sendData(data);
	}
	
	public void sendAllDataExcept(int[] daemons, KV data) throws IOException {
		List<Integer> daemonsIgnored = new ArrayList<>();
		for (int daemon : daemons)
			daemonsIgnored.add(daemon);
		for (int i = 0; i < this.numberDaemons; i++) 
	...
	public void sendAllDataExcept(int daemon, KV data) throws IOException {
		for (int i = 0; i < this.numberDaemons; i++) 
			if (i != daemon)
				this.daemonStreams[i].sendData(data);
	}
