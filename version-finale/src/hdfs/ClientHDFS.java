...
public class ClientHDFS {
			...
			while (!error && commandIterator.hasNext()) {
				...
				if (command.requiresFileName()) {
    				...
    				while (!error && fileIterator.hasNext()) {
    					...
    					File file = new File(fileDescription.getPath() + fileDescription.getName());
    					if (file.exists()) {
    ...
    private static void printUse() {
    	...
    	System.out.println("Le programme ne prend qu'un seul paramètre. Ce paramètre peut contenir plusieurs "
    			...
    			+ "d'opérations contient soit la liste des commandes seule (commandes serveurs uniqument), soit "
    			+ "la liste des commandes suivie de la liste des fichiers sur lesquels effectuer les commandes "
    ...
    private static void printFileUse() {
    	...
    	System.out.println("Un fichier peut être décrit en plus de par son nom par un alias (option -a) et par un nom de "
    	...
    	System.out.println("L'alias permet de désigner un fichier sur le serveur. Les alias sont uniques et le serveur ne "
    			...
    			+ "effectuer le traitement par son alias respectif sinon les opérations ne pourront pas être effectués sur ce "
    			+ "fichier. L'alias peut être utilisé explicitement (option -a) ou implicitement à la place du nom principal. "
    			+ "L'alias sera utilisé en priorité pour trouver le fichier sur le serveur sur lequel effectué le traitement.");
    	System.out.println("L'alias est enregistré s'il est utilisé explicitement avec le fichier lorsque le celui-ci est "
    			+ "envoyé sur le serveur (commande 1) pour la première fois. Par la suite l'alias peut être modifié en mettant "
    			+ "à jour la description du fichier sur le serveur (commande 4). Le fichier associé peut ensuite être récupéré "
    			+ "(commande 2) en utilisant son alias.");
    	...
    	System.out.println("java hdfs.ClientHDFS '1, test.txt -a testBis'       - Deuxième enregistrement du fichier test.txt avec l'alias 'testBis'");
    	System.out.println("java hdfs.ClientHDFS '2, test -d testCopie1.txt'    - Récupération du fichier test.txt avec l'alias 'test' sous le nom testCopie1.txt");
    	...
    	System.out.println("java hdfs.ClientHDFS '2, testBis -d testCopie2.txt' - Récupération du fichier test.txt avec l'alias 'testBis' sous le nom testCopie2.txt");
    	System.out.println();
