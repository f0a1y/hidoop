...
public class Cluster {
	...
	public byte[] receiveData(int id) throws IOException {
		return this.nodesStreams[id].receiveData();
	}
	
	public int receiveDataInt(int id) throws IOException {
		return this.nodesStreams[id].receiveDataInt();
	}
	
	public void sendAllData(byte[] data, int length) throws IOException {
		for (int i = 0; i < this.numberNodes; i++) {
			this.nodesStreams[i].sendData(data, length);
		}
	}
	
	public void sendData(int id, byte[]data, int length) throws IOException {
		this.nodesStreams[id].sendData(data, length);
	}
	
	public void sendAllData(int data) throws IOException {
		for (int i = 0; i < this.numberNodes; i++) {
			this.nodesStreams[i].sendData(data);
		}
	}
	
	public void sendData(int id, int data) throws IOException {
		this.nodesStreams[id].sendData(data);
	}
